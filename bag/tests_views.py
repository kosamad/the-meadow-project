...
class TestBagView(TestCase):
    ...
    def test_add_product_to_bag(self):
        response = self.client.post(
            reverse('add_product_to_bag', args=[self.product.id]),
            {
                'quantity': 1,
        ...
        unique_key = (
            f"{self.product.id}_{self.variant.id}"
            "_Happy Birthday_No green"
        )
    ...
    def test_update_card_message(self):
        ...
        response = self.client.post(
            reverse('update_card_message', args=[self.product.id]),
            {
